...
public class ProgressDialog extends JDialog implements Progress {
		...
		Runnable cancelableAction)
	{
		...
		detailsToggle.addActionListener((
			final ActionEvent event) -> toggleDetails());
		buttons.add(detailsToggle);
		final JButton cancel = new JButton("Cancel");
		cancel.addActionListener((final ActionEvent e) -> {
			canceled = true;
	...
	protected void setTitle() {
		...
		SwingTools.invokeOnEDT(() -> {
			if (detailsScrollPane.isVisible() || latestDetail == null) progress
				.setString(windowTitle);
			else progress.setString(windowTitle + ": " + latestDetail.getString());
		});
		repaint();
	}
