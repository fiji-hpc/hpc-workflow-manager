...
class SnakemakeOutputHelper implements HaaSOutputHolder {
	...
	List<HPCWorkflowError> getErrors() {
		getTasks();
		final Stream<HPCWorkflowError> taskSpecificErrors = tasks.stream().flatMap(
			s -> s.getErrors().stream());
		return Stream.concat(nonTaskSpecificErrors.stream(), taskSpecificErrors)
			.collect(Collectors.toList());
	}

	private ComputationAccessor createComputationAccessor() {
		ComputationAccessor result = new ComputationAccessor() {
